...
if st.session_state.page == "タスク一覧":
    ...
    tasks_df = da.get_tasks_from_supabase()
    ...
    if tasks_df.empty:
        ...
        if sort_key == "期日が近い順":
            # '期日までの日数' を数値に変換し、変換できないものはNaNにする
            display_df['sort_key'] = pd.to_numeric(display_df['期日までの日数'], errors='coerce')
            # na_position='last'でNaN（期限切れや空欄）を末尾に持ってくる
    ...
    if not unregistered_universities:
        ...
        if st.button("この大学のタスクを追加する", type="primary"):
            if selected_university:
                success = da.add_university_tasks_to_supabase(selected_university)
                if success:
